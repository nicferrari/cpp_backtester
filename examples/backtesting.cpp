...
int main() {
    ...
    //results5.print(ts);
    Metrics metrics = results.toMetrics(ts);
    Metrics metrics2 = results2.toMetrics(ts);
    Metrics metrics3 = results3.toMetrics(ts);
    Metrics metrics4 = results4.toMetrics(ts);
    Metrics metrics5 = results5.toMetrics(ts);
    compareResults({metrics,metrics2, metrics3, metrics4, metrics5});
    return 0;
}